...
extern "C" void dynopt_interp8_hps_16x32_sve2(const uint8_t* src, intptr_t srcStride, int16_t* dst, intptr_t dstStride, int coeffIdx, int isRowExt)
{
    ...
    for (int r = 0; r < 32; r++)
    {
        for (int g = 0; g < 2; g++)
        {
            svuint8_t s = svld1_u8(pg16b, src + (r + 0) * srcStride -3 + g * 8);
