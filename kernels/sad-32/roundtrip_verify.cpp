...
// SAD (assembly reference) and a plain C reference (32x32, u8).
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
#define DYNOPT_CANDIDATE dynopt_sad_32x32_neon_roundtrip
#endif
extern "C" int DYNOPT_CANDIDATE(
    const uint8_t* a, intptr_t sa, const uint8_t* b, intptr_t sb);
extern "C" int x265_pixel_sad_32x32_neon_dotprod(
    const uint8_t* a, intptr_t sa, const uint8_t* b, intptr_t sb);
...
static std::vector<uint8_t> make_plane(int stride, int off,
                                       const uint8_t* vals)
{
    std::vector<uint8_t> p((size_t)stride * 32 + off, 0xAA);
    for (int r = 0; r < 32; r++)
        memcpy(p.data() + off + (size_t)r * stride, vals + r * 32, 32);
    return p;
}
...
int main(int argc, char** argv)
{
    ...
    std::mt19937 rng(0x332D2026u);
    // stride must be >= row width (32) or make_plane overflows.
    int strides[5] = { 32, 33, 64, 65, 96 };
    int offs[4] = { 0, 1, 3, 7 };
    int mism = 0;
    for (int i = 0; i < cases; i++)
    {
        uint8_t va[1024], vb[1024];
        ...
        int rn = x265_pixel_sad_32x32_neon_dotprod(a, sa, b, sb);
