// SATD 16x4 SVE2 candidate: native SVE2 cadd butterfly (950-only).
//
// 1 half-vectors per row x 1 4-row quads of the verified satd-16 cadd
// kernel; gate-arbitrated vs x265::satd8_sve2<16,4>.
...
extern "C" int dynopt_satd_16x4_sve2(const uint8_t* pix1, intptr_t sp1,
              const uint8_t* pix2, intptr_t sp2)
{
    ...
    for (int g = 0; g < 1; g++)
    {
        ...
        svint16_t gs = svadd_s16_x(p16, m0a, m1a);
