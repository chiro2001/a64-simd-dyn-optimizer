...
// x265_nquant_neon (open-source NEON asm), numCoeff=256.
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <random>
...
#define DYNOPT_CANDIDATE dynopt_nquant_256_roundtrip
#endif
extern "C" uint32_t DYNOPT_CANDIDATE(
    const int16_t*, const int32_t*, int16_t*, int, int);
extern "C" uint32_t x265_nquant_neon(
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < cases; i++)
    {
        ...
        const uint32_t s1 = x265_nquant_neon(
            coef.data(), qc.data(), q1.data(), qBits, add, num);
        const uint32_t s2 = DYNOPT_CANDIDATE(
            coef.data(), qc.data(), q2.data(), qBits, add);
        if (s1 != s2)
        {
            if (mism < 5)
        ...
        for (int j = 0; j < num; j++)
            ...
            {
                if (mism < 5)
                    fprintf(stderr, "mismatch %d[%d]: q ref=%d gen=%d\n",
                            i, j, q1[j], q2[j]);
                mism++;
                break;
            }
