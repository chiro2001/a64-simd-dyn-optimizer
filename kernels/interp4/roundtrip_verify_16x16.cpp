// Differential probe for the generated interp4 16x16 roundtrip candidate
// against the upstream x265::interp4_horiz_pp_dotprod<16,16> reference.
// Phase 4 (identity filter) is excluded: the seed covers the dotprod path,
// phase==4 dispatches to another kernel (project contract: 20k x 7 phases).
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
extern "C" void dynopt_interp4_16x16_neon_roundtrip(
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < cases; i++)
    {
        ...
        int ph = (int)(rng() % 8);
        if (ph == 4)
            ph = 1;  // skip the identity-filter phase
        uint8_t src[16 * 64 + 32], wa[16 * 64], wb[16 * 64];
        for (int j = 0; j < (int)sizeof(src); j++)
            src[j] = (uint8_t)(rng() % 256);
        x265::interp4_horiz_pp_dotprod<16, 16>(src, ss, wa, ds, ph);
        dynopt_interp4_16x16_neon_roundtrip(src, ss, wb, ds, ph);
        for (int r = 0; r < 16; r++)
