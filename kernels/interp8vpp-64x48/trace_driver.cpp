...
// vpp (vertical 8-tap) 64x48 candidate.
#include <cstdint>
#include <cstring>

extern "C" void dynopt_interp8_64x48_sve2_vpp(
    const uint8_t*, intptr_t, uint8_t*, intptr_t, int)
    __attribute__((noinline));

int main()
{
    static uint8_t src[(48 + 6) * 128 + 32];
    static uint8_t dst[48 * 128];
    for (int i = 0; i < (int)sizeof(src); i++)
        src[i] = (uint8_t)((i * 37 + 11) % 256);
    memset(dst, 0, sizeof(dst));
    dynopt_interp8_64x48_sve2_vpp(src + 3 * 128 + 16, 128, dst, 128, 2);
    int sum = 0;
