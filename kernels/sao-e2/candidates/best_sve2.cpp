// SAO edge offset class 2 (135deg diagonal), 64x1 SVE2 candidate.
//
// Port of processSaoCUE2_neon: signDown = sign(rec[x] - rec[x+stride+1])
// (down-right diagonal), edgeType = signDown + buff1[x] + 2, bufft[x+1]
// = -signDown (shifted store), rec[x] = saturate(rec[x] +
// eoTable[edgeType]). Single 64-pixel row (the x265 loop calls per row
// and double-buffers bufft/buff1 across rows).
//
// Gate-arbitrated bit-exact vs processSaoCUE2_neon (64x1, bufft[1..64]
// compared).
...
extern "C" void dynopt_sao_e2_64_sve2(uint8_t* rec, int8_t* bufft,
                                      int8_t* buff1,
