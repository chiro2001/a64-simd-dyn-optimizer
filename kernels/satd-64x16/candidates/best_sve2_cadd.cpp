// SATD 64x16 SVE2 candidate: native SVE2 cadd butterfly (950-only).
//
// Horizontal extension x4 of the verified satd-16 cadd kernel: each
// 64-col row = four 16-lane vectors, ROWH4 butterfly on each half
// independently, vertical pair abs/abd + max folded per half, totals
// summed. Gate-arbitrated vs x265::satd8_sve2<64,16>.
...
extern "C" int dynopt_satd_64x16_sve2(const uint8_t* pix1, intptr_t sp1,
              const uint8_t* pix2, intptr_t sp2)
{
    ...
    for (int g = 0; g < 4; g++)
    {
        ...
        svint16_t a0c, d0c, a1c, d1c, a0d, d0d, a1d, d1d;
        ...
        svint16_t gs = svadd_s16_x(p16,
            svadd_s16_x(p16, svadd_s16_x(p16, m0a, m1a),
                        svadd_s16_x(p16, m0b, m1b)),
            svadd_s16_x(p16, svadd_s16_x(p16, m0c, m1c),
                        svadd_s16_x(p16, m0d, m1d)));
