// SATD 32x32 SVE2 candidate: native SVE2 cadd butterfly (950-only).
//
// Horizontal extension of the verified satd-16 cadd kernel: each 32-col
// row = two 16-lane vectors, ROWH4 butterfly on each half independently,
// vertical pair abs/abd + max folded per half, totals summed. Whether
// this matches x265::satd8_sve2<32,32> (independent halves vs cross
// stage) is arbitrated by the QEMU funnel gate.
...
extern "C" int dynopt_satd_32x32_sve2(const uint8_t* pix1, intptr_t sp1,
              const uint8_t* pix2, intptr_t sp2)
{
    ...
    for (int g = 0; g < 8; g++)
    {
        ...
        svint16_t gs = svadd_s16_x(p16, svadd_s16_x(p16, m0a, m1a),
