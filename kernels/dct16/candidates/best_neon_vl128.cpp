// Generated by tools/emit_dct16_vl128.py -- do not edit by hand.
// VL=128 migration baseline: upstream 8-lane E/O + sdot structure
// (upstream-exact at any SVE VL; structural axes applied later).
// isa=neon pass1=upstream pass2=upstream

// Depth-8 contract, matching the gen_verify legacy oracle.
#define X265_DEPTH 8
#include <arm_neon.h>
#include <cstdint>


const int16_t t8_odd[4][8] =
{
    { 89,  75,  50,  18, 89,  75,  50,  18 },
    { 75, -18, -89, -50, 75, -18, -89, -50 },
    { 50, -89,  18,  75, 50, -89,  18,  75 },
    { 18, -50,  75, -89, 18, -50,  75, -89 },
};

const int32_t t8_even[4][4] =
{
    { 64,  64, 64,  64 },
    { 83,  36, 83,  36 },
    { 64, -64, 64, -64 },
    { 36, -83, 36, -83 },
};

const uint8_t rev16_tbl[16] =
{
    14, 15, 12, 13, 10, 11, 8, 9, 6, 7, 4, 5, 2, 3, 0, 1
};

const uint8_t rev32_tbl[16] =
{
    12, 13, 14, 15, 8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3
};

const int16_t g_t16[16][16] =
{
    { 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64 },
    { 90, 87, 80, 70, 57, 43, 25,  9, -9, -25, -43, -57, -70, -80, -87, -90 },
    { 89, 75, 50, 18, -18, -50, -75, -89, -89, -75, -50, -18, 18, 50, 75, 89 },
    { 87, 57,  9, -43, -80, -90, -70, -25, 25, 70, 90, 80, 43, -9, -57, -87 },
    { 83, 36, -36, -83, -83, -36, 36, 83, 83, 36, -36, -83, -83, -36, 36, 83 },
    { 80,  9, -70, -87, -25, 57, 90, 43, -43, -90, -57, 25, 87, 70, -9, -80 },
    { 75, -18, -89, -50, 50, 89, 18, -75, -75, 18, 89, 50, -50, -89, -18, 75 },
    { 70, -43, -87,  9, 90, 25, -80, -57, 57, 80, -25, -90, -9, 87, 43, -70 },
    { 64, -64, -64, 64, 64, -64, -64, 64, 64, -64, -64, 64, 64, -64, -64, 64 },
    { 57, -80, -25, 90, -9, -87, 43, 70, -70, -43, 87,  9, -90, 25, 80, -57 },
    { 50, -89, 18, 75, -75, -18, 89, -50, -50, 89, -18, -75, 75, 18, -89, 50 },
    { 43, -90, 57, 25, -87, 70,  9, -80, 80, -9, -70, 87, -25, -57, 90, -43 },
    { 36, -83, 83, -36, -36, 83, -83, 36, 36, -83, 83, -36, -36, 83, -83, 36 },
    { 25, -70, 90, -80, 43,  9, -57, 87, -87, 57, -9, -43, 80, -90, 70, -25 },
    { 18, -50, 75, -89, 89, -75, 50, -18, -18, 50, -75, 89, -89, 75, -50, 18 },
    {  9, -25, 43, -57, 70, -80, 87, -90, 90, -87, 80, -70, 57, -43, 25, -9 }
};

static inline int16x8_t rev16(const int16x8_t a)
{
    const uint8x16_t tbl = vld1q_u8(rev16_tbl);
    const int8x16_t a_s8 = vreinterpretq_s8_s16(a);

    return vreinterpretq_s16_s8(vqtbx1q_s8(a_s8, a_s8, tbl));
}

static inline int32x4_t rev32(const int32x4_t a)
{
    const uint8x16_t tbl = vld1q_u8(rev32_tbl);
    const int8x16_t a_s8 = vreinterpretq_s8_s32(a);

    return vreinterpretq_s32_s8(vqtbx1q_s8(a_s8, a_s8, tbl));
}



static inline void pass1Butterfly16_sve(const int16_t *src, int16_t *dst,
                                        intptr_t srcStride)
{
    const int shift = 3 + X265_DEPTH - 8;
    const int line = 16;

    for (int i = 0; i < line; i += 4)
    {
        int16x8_t s0_lo = vld1q_s16(src + (i + 0) * srcStride);
        int16x8_t s0_hi = rev16(vld1q_s16(src + (i + 0) * srcStride + 8));
        int16x8_t s1_lo = vld1q_s16(src + (i + 1) * srcStride);
        int16x8_t s1_hi = rev16(vld1q_s16(src + (i + 1) * srcStride + 8));
        int16x8_t s2_lo = vld1q_s16(src + (i + 2) * srcStride);
        int16x8_t s2_hi = rev16(vld1q_s16(src + (i + 2) * srcStride + 8));
        int16x8_t s3_lo = vld1q_s16(src + (i + 3) * srcStride);
        int16x8_t s3_hi = rev16(vld1q_s16(src + (i + 3) * srcStride + 8));

        int16x8_t O0 = vsubq_s16(s0_lo, s0_hi);
        int16x8_t O1 = vsubq_s16(s1_lo, s1_hi);
        int16x8_t O2 = vsubq_s16(s2_lo, s2_hi);
        int16x8_t O3 = vsubq_s16(s3_lo, s3_hi);

        int32x4_t s0_0 = vaddl_s16(vget_low_s16(s0_lo),
                                      vget_low_s16(s0_hi));
        int32x4_t s0_1 = vaddl_s16(vget_high_s16(s0_lo),
                                      vget_high_s16(s0_hi));
        int32x4_t s1_0 = vaddl_s16(vget_low_s16(s1_lo),
                                      vget_low_s16(s1_hi));
        int32x4_t s1_1 = vaddl_s16(vget_high_s16(s1_lo),
                                      vget_high_s16(s1_hi));
        int16x8_t EO0 = vcombine_s16(
            vmovn_s32(vsubq_s32(s0_0, rev32(s0_1))),
            vmovn_s32(vsubq_s32(s1_0, rev32(s1_1))));
        int32x4_t e0_0 = vaddq_s32(s0_0, rev32(s0_1));
        int32x4_t e0_1 = vaddq_s32(s1_0, rev32(s1_1));
        int32x4_t t0_0 = vreinterpretq_s32_s64(
            vzip1q_s64(vreinterpretq_s64_s32(e0_0),
                       vreinterpretq_s64_s32(e0_1)));
        int32x4_t t0_1 = vrev64q_s32(vreinterpretq_s32_s64(
            vzip2q_s64(vreinterpretq_s64_s32(e0_0),
                       vreinterpretq_s64_s32(e0_1))));
        int32x4_t EEE0 = vaddq_s32(t0_0, t0_1);
        int32x4_t EEO0 = vsubq_s32(t0_0, t0_1);
        int32x4_t s2_0 = vaddl_s16(vget_low_s16(s2_lo),
                                      vget_low_s16(s2_hi));
        int32x4_t s2_1 = vaddl_s16(vget_high_s16(s2_lo),
                                      vget_high_s16(s2_hi));
        int32x4_t s3_0 = vaddl_s16(vget_low_s16(s3_lo),
                                      vget_low_s16(s3_hi));
        int32x4_t s3_1 = vaddl_s16(vget_high_s16(s3_lo),
                                      vget_high_s16(s3_hi));
        int16x8_t EO1 = vcombine_s16(
            vmovn_s32(vsubq_s32(s2_0, rev32(s2_1))),
            vmovn_s32(vsubq_s32(s3_0, rev32(s3_1))));
        int32x4_t e1_0 = vaddq_s32(s2_0, rev32(s2_1));
        int32x4_t e1_1 = vaddq_s32(s3_0, rev32(s3_1));
        int32x4_t t1_0 = vreinterpretq_s32_s64(
            vzip1q_s64(vreinterpretq_s64_s32(e1_0),
                       vreinterpretq_s64_s32(e1_1)));
        int32x4_t t1_1 = vrev64q_s32(vreinterpretq_s32_s64(
            vzip2q_s64(vreinterpretq_s64_s32(e1_0),
                       vreinterpretq_s64_s32(e1_1))));
        int32x4_t EEE1 = vaddq_s32(t1_0, t1_1);
        int32x4_t EEO1 = vsubq_s32(t1_0, t1_1);

        for (int k = 1; k < 16; k += 2)
        {
            int16x8_t c0_c4 = vld1q_s16(&g_t16[k][0]);
            int32x4_t a0 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O0));
            a0 = vmlal_s16(a0, vget_high_s16(c0_c4), vget_high_s16(O0));
            int32x4_t a1 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O1));
            a1 = vmlal_s16(a1, vget_high_s16(c0_c4), vget_high_s16(O1));
            int32x4_t a2 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O2));
            a2 = vmlal_s16(a2, vget_high_s16(c0_c4), vget_high_s16(O2));
            int32x4_t a3 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O3));
            a3 = vmlal_s16(a3, vget_high_s16(c0_c4), vget_high_s16(O3));
            int32x4_t s01 = vpaddq_s32(a0, a1);
            int32x4_t s23 = vpaddq_s32(a2, a3);
            int32x4_t t = vpaddq_s32(s01, s23);
            int16x4_t res = vrshrn_n_s32(t, shift);
            vst1_s16(dst + k * line, res);

        }

        for (int k = 2; k < 16; k += 4)
        {
            int16x8_t c0 = vld1q_s16(t8_odd[(k - 2) / 4]);
            int32x4_t p0 = vmull_s16(vget_low_s16(c0), vget_low_s16(EO0));
            int32x4_t p1 = vmull_s16(vget_high_s16(c0), vget_high_s16(EO0));
            int32x4_t p2 = vmull_s16(vget_low_s16(c0), vget_low_s16(EO1));
            int32x4_t p3 = vmull_s16(vget_high_s16(c0), vget_high_s16(EO1));
            int32x4_t s01 = vpaddq_s32(p0, p1);
            int32x4_t s23 = vpaddq_s32(p2, p3);
            int32x4_t t = vpaddq_s32(s01, s23);
            int16x4_t res = vrshrn_n_s32(t, shift);
            vst1_s16(dst + k * line, res);

        }

        int32x4_t c0 = vld1q_s32(t8_even[0]);
        int32x4_t c4 = vld1q_s32(t8_even[1]);
        int32x4_t c8 = vld1q_s32(t8_even[2]);
        int32x4_t c12 = vld1q_s32(t8_even[3]);

        int32x4_t t0 = vpaddq_s32(EEE0, EEE1);
        int32x4_t t1 = vmulq_s32(c0, t0);
        int16x4_t res0 = vrshrn_n_s32(t1, shift);
        vst1_s16(dst + 0 * line, res0);

        int32x4_t t2 = vmulq_s32(c4, EEO0);
        int32x4_t t3 = vmulq_s32(c4, EEO1);
        int16x4_t res4 = vrshrn_n_s32(vpaddq_s32(t2, t3), shift);
        vst1_s16(dst + 4 * line, res4);

        int32x4_t t4 = vmulq_s32(c8, EEE0);
        int32x4_t t5 = vmulq_s32(c8, EEE1);
        int16x4_t res8 = vrshrn_n_s32(vpaddq_s32(t4, t5), shift);
        vst1_s16(dst + 8 * line, res8);

        int32x4_t t6 = vmulq_s32(c12, EEO0);
        int32x4_t t7 = vmulq_s32(c12, EEO1);
        int16x4_t res12 = vrshrn_n_s32(vpaddq_s32(t6, t7), shift);
        vst1_s16(dst + 12 * line, res12);

        dst += 4;
    }
}


static inline void pass2Butterfly16_sve(const int16_t *src, int16_t *dst)
{
    const int shift = 10;
    const int line = 16;

    for (int i = 0; i < line; i += 4)
    {
        int16x8_t s0_lo = vld1q_s16(src + (i + 0) * line);
        int16x8_t s0_hi = rev16(vld1q_s16(src + (i + 0) * line + 8));
        int16x8_t s1_lo = vld1q_s16(src + (i + 1) * line);
        int16x8_t s1_hi = rev16(vld1q_s16(src + (i + 1) * line + 8));
        int16x8_t s2_lo = vld1q_s16(src + (i + 2) * line);
        int16x8_t s2_hi = rev16(vld1q_s16(src + (i + 2) * line + 8));
        int16x8_t s3_lo = vld1q_s16(src + (i + 3) * line);
        int16x8_t s3_hi = rev16(vld1q_s16(src + (i + 3) * line + 8));

        int16x8_t O0 = vsubq_s16(s0_lo, s0_hi);
        int16x8_t O1 = vsubq_s16(s1_lo, s1_hi);
        int16x8_t O2 = vsubq_s16(s2_lo, s2_hi);
        int16x8_t O3 = vsubq_s16(s3_lo, s3_hi);

        int32x4_t s0_0 = vaddl_s16(vget_low_s16(s0_lo),
                                      vget_low_s16(s0_hi));
        int32x4_t s0_1 = vaddl_s16(vget_high_s16(s0_lo),
                                      vget_high_s16(s0_hi));
        int32x4_t s1_0 = vaddl_s16(vget_low_s16(s1_lo),
                                      vget_low_s16(s1_hi));
        int32x4_t s1_1 = vaddl_s16(vget_high_s16(s1_lo),
                                      vget_high_s16(s1_hi));
        int32x4_t EO0_a = vsubq_s32(s0_0, rev32(s0_1));
        int32x4_t EO0_b = vsubq_s32(s1_0, rev32(s1_1));
        int32x4_t e0_0 = vaddq_s32(s0_0, rev32(s0_1));
        int32x4_t e0_1 = vaddq_s32(s1_0, rev32(s1_1));
        int32x4_t t0_0 = vreinterpretq_s32_s64(
            vzip1q_s64(vreinterpretq_s64_s32(e0_0),
                       vreinterpretq_s64_s32(e0_1)));
        int32x4_t t0_1 = vrev64q_s32(vreinterpretq_s32_s64(
            vzip2q_s64(vreinterpretq_s64_s32(e0_0),
                       vreinterpretq_s64_s32(e0_1))));
        int32x4_t EEE0 = vaddq_s32(t0_0, t0_1);
        int32x4_t EEO0 = vsubq_s32(t0_0, t0_1);
        int32x4_t s2_0 = vaddl_s16(vget_low_s16(s2_lo),
                                      vget_low_s16(s2_hi));
        int32x4_t s2_1 = vaddl_s16(vget_high_s16(s2_lo),
                                      vget_high_s16(s2_hi));
        int32x4_t s3_0 = vaddl_s16(vget_low_s16(s3_lo),
                                      vget_low_s16(s3_hi));
        int32x4_t s3_1 = vaddl_s16(vget_high_s16(s3_lo),
                                      vget_high_s16(s3_hi));
        int32x4_t EO1_a = vsubq_s32(s2_0, rev32(s2_1));
        int32x4_t EO1_b = vsubq_s32(s3_0, rev32(s3_1));
        int32x4_t e1_0 = vaddq_s32(s2_0, rev32(s2_1));
        int32x4_t e1_1 = vaddq_s32(s3_0, rev32(s3_1));
        int32x4_t t1_0 = vreinterpretq_s32_s64(
            vzip1q_s64(vreinterpretq_s64_s32(e1_0),
                       vreinterpretq_s64_s32(e1_1)));
        int32x4_t t1_1 = vrev64q_s32(vreinterpretq_s32_s64(
            vzip2q_s64(vreinterpretq_s64_s32(e1_0),
                       vreinterpretq_s64_s32(e1_1))));
        int32x4_t EEE1 = vaddq_s32(t1_0, t1_1);
        int32x4_t EEO1 = vsubq_s32(t1_0, t1_1);

        for (int k = 1; k < 16; k += 2)
        {
            int16x8_t c0_c4 = vld1q_s16(&g_t16[k][0]);
            int32x4_t a0 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O0));
            a0 = vmlal_s16(a0, vget_high_s16(c0_c4), vget_high_s16(O0));
            int32x4_t a1 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O1));
            a1 = vmlal_s16(a1, vget_high_s16(c0_c4), vget_high_s16(O1));
            int32x4_t a2 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O2));
            a2 = vmlal_s16(a2, vget_high_s16(c0_c4), vget_high_s16(O2));
            int32x4_t a3 = vmull_s16(vget_low_s16(c0_c4), vget_low_s16(O3));
            a3 = vmlal_s16(a3, vget_high_s16(c0_c4), vget_high_s16(O3));
            int32x4_t s01 = vpaddq_s32(a0, a1);
            int32x4_t s23 = vpaddq_s32(a2, a3);
            int32x4_t t = vpaddq_s32(s01, s23);
            int16x4_t res = vrshrn_n_s32(t, shift);
            vst1_s16(dst + k * line, res);

        }

        for (int k = 2; k < 16; k += 4)
        {
            int32x4_t c0 = vmovl_s16(vld1_s16(&g_t16[k][0]));

            int32x4_t t0 = vmulq_s32(c0, EO0_a);
            int32x4_t t1 = vmulq_s32(c0, EO0_b);
            int32x4_t t2 = vmulq_s32(c0, EO1_a);
            int32x4_t t3 = vmulq_s32(c0, EO1_b);
            int32x4_t t = vpaddq_s32(vpaddq_s32(t0, t1),
                                     vpaddq_s32(t2, t3));

            int16x4_t res = vrshrn_n_s32(t, shift);
            vst1_s16(dst + k * line, res);
        }

        int32x4_t c0 = vld1q_s32(t8_even[0]);
        int32x4_t c4 = vld1q_s32(t8_even[1]);
        int32x4_t c8 = vld1q_s32(t8_even[2]);
        int32x4_t c12 = vld1q_s32(t8_even[3]);

        int32x4_t t0 = vpaddq_s32(EEE0, EEE1);
        int32x4_t t1 = vmulq_s32(c0, t0);
        int16x4_t res0 = vrshrn_n_s32(t1, shift);
        vst1_s16(dst + 0 * line, res0);

        int32x4_t t2 = vmulq_s32(c4, EEO0);
        int32x4_t t3 = vmulq_s32(c4, EEO1);
        int16x4_t res4 = vrshrn_n_s32(vpaddq_s32(t2, t3), shift);
        vst1_s16(dst + 4 * line, res4);

        int32x4_t t4 = vmulq_s32(c8, EEE0);
        int32x4_t t5 = vmulq_s32(c8, EEE1);
        int16x4_t res8 = vrshrn_n_s32(vpaddq_s32(t4, t5), shift);
        vst1_s16(dst + 8 * line, res8);

        int32x4_t t6 = vmulq_s32(c12, EEO0);
        int32x4_t t7 = vmulq_s32(c12, EEO1);
        int16x4_t res12 = vrshrn_n_s32(vpaddq_s32(t6, t7), shift);
        vst1_s16(dst + 12 * line, res12);

        dst += 4;
    }
}


extern "C" void dynopt_dct16_sve2_shared(const int16_t* src, int16_t* dst,
                                         intptr_t srcStride)
{
    int16_t coef[16 * 16];
    pass1Butterfly16_sve(src, coef, srcStride);
    pass2Butterfly16_sve(coef, dst);
}
