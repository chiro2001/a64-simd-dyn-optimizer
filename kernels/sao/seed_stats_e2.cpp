// SAO stats E2 (diagonal 135deg), 64x1 NEON seed (ACLE, docs/45 §9).
// Mirrors saoCuStatsE2_neon: upBuff1 holds -sign_up (negated on entry),
// upBufft[0] = sign(rec[-1]-rec[stride]), then per pixel
// sign_down = sign(rec[x]-rec[x+stride+1]), edge_type = sign_down -
// sign_up, upBufft[x+1] = sign_down; 5 accumulators + reduce.
#include <arm_neon.h>
...
extern "C" void dynopt_sao_stats_e2_64(
    const int16_t* diff, const uint8_t* rec, intptr_t stride,
    int8_t* upBuff1, int8_t* upBufft, int32_t* stats, int32_t* count)
{
