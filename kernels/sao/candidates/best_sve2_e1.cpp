// SVE2 saoCuOrgE1 (64x4, vertical), upstream-exact vs x265 C
// semantics: edge = signDown + upBuff + 2; upBuff = -signDown;
// out = clip(rec + offset[edge]).
//
// The edge/upBuff recurrence is across rows only, not across x, so all
// 32 byte lanes are independent within a row.  Same 128-bias clip as B0.
#include <arm_sve.h>
#include <stdint.h>
