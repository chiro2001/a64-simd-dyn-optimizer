// SAO edge offset class 1 (vertical), 64x4 NEON seed (ACLE, docs/45).
// Mirrors processSaoCUE1_neon per row: edgeType = signDown + signUp + 2,
// upBuff1[i] = -signDown, rec = clip(rec + offsetEo[edgeType]).
#include <arm_neon.h>

#include <stddef.h>
#include <stdint.h>

static inline int8x8_t sao_sign1(uint8x8_t a, uint8x8_t b)
{
...
#define E1_ROW(y)                                                         \
    do {                                                                  \
        E1_BLOCK(y, 0);   E1_BLOCK(y, 8);   E1_BLOCK(y, 16);              \
        E1_BLOCK(y, 24);  E1_BLOCK(y, 32);  E1_BLOCK(y, 40);              \
        E1_BLOCK(y, 48);  E1_BLOCK(y, 56);                                \
    } while (0)

extern "C" void dynopt_sao_e1_64x4(
