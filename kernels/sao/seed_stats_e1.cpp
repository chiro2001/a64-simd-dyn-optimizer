// SAO stats E1, 64x1 NEON seed (ACLE, docs/45 §8). Mirrors
// saoCuStatsE1_neon: upBuff1 holds -sign_up (negated on entry), then
// edge_type = sign_down - sign_up; 5 edge-type count/stats accumulators,
// reduce; upBuff1[x] updated to sign_down for the next row.
#include <arm_neon.h>
    ...
    int8_t* upBuff1, int32_t* stats, int32_t* count)
{
    ...
    // upBuff1 holds -sign_up; negate in place so sign_up = -loaded.
