...
def emit_c_intrinsics(machine_ir, func_name="dynopt_sa8d_8x8_neon_roundtrip",
    ...
    for node in machine_ir.nodes:
        ...
        if op == "icmp":
            ...
            stride = {"pix1": "stride_pix1", "pix2": "stride_pix2",
                      "p1": "s1", "p2": "s2"}.get(base)
            if stride:
                if byte:
            ...
            if src is not None and str(src) in mask_nodes:
                # sext of an icmp mask (0/-1): alias the mask variable
                cname[dst] = cid(src)
                types[dst] = types.get(cid(src))
            elif vtype == "<4 x i32>":
            ...
            if len(node["src"]) == 2:
                ...
                types[dst] = ("uint%d%s_t"
                              % (int(suffix[1:]),
                                 "x%d" % int(re.match(
                                     r"<(\d+) x", vtype).group(1)))
                              if unsigned else TYPE_MAP[vtype])
            ...
            if name == "abs":
                ...
                types[dst] = "uint32x4_t"
                if types.get(str(node["src"][0])) == "uint16x8_t":
                ...
                if types.get(str(node["src"][0])) == "uint32x4_t":
                    types[dst] = "uint32x4_t"
