"""Bounded cover search for satd 32x32 (AGO M3 extension).
...
def cover_meta() -> Dict:
    # SATD 32x32 dataflow: load -> sub/u16 -> horizontal 4-point cadd
...
def emit_cover(cover: str, func_name: str = "dynopt_satd_32x32_sve2") -> str:
    """Return the satd-32x32 candidate source (symbol defined in file)."""
    if cover not in _FILES:
        raise ValueError("satd-32x32 cover %s not defined" % cover)
    import os

    _ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    p = os.path.join(_ROOT, "kernels", "satd-32x32", "candidates",
                     _FILES[cover])
    with open(p) as f:
        return f.read()
