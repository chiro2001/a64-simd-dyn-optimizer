...
def cover_meta() -> Dict:
    # SATD 64x16 dataflow: load -> sub/u16 -> horizontal 4-point cadd
...
def emit_cover(cover: str, func_name: str = "dynopt_satd_64x16_sve2") -> str:
    """Return the satd-64x16 candidate source (symbol defined in file)."""
    if cover not in _FILES:
        raise ValueError("satd-64x16 cover %s not defined" % cover)
    import os

    _ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    p = os.path.join(_ROOT, "kernels", "satd-64x16", "candidates",
                     _FILES[cover])
    with open(p) as f:
        return f.read()
