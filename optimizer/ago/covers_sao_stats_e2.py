"""Bounded cover search for sao-stats-e2 (AGO M3 extension).

Covers (all export dynopt_sao_stats_e2_64_sve2):

  A best_sve2.cpp : best_sve2.cpp (fused=)
  B block16_sve2.cpp : block16_sve2.cpp (fused=)
  C block32_sve2.cpp : block32_sve2.cpp (fused=)

Gate-arbitrated bit-exact vs the saoCuStatsE2_neon reference.
"""
...
def emit_cover(cover: str, func_name: str = "dynopt_sao_stats_e2_64_sve2") -> str:
    """Return an existing sao-stats-e2 candidate source."""
    if cover not in _FILES:
        raise ValueError("sao-stats-e2 cover %s not defined" % cover)
    import os

    _ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    p = os.path.join(_ROOT, "kernels", "sao-stats-e2", "candidates",
                     _FILES[cover])
    with open(p) as f:
        return f.read()
