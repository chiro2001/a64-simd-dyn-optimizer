"""Bounded cover search for satd 16x32 (AGO M3 extension).
...
def cover_meta() -> Dict:
    # SATD 16x32 dataflow: load -> sub/u16 -> horizontal 4-point cadd
    # butterfly (cadd + tbl reorder) -> vertical pair abs/abd -> max ->
    # per-quad sum -> total (8 quads).
...
def emit_cover(cover: str, func_name: str = "dynopt_satd_16x32_sve2") -> str:
    """Return the satd-16x32 candidate source (symbol defined in file)."""
    if cover not in _FILES:
        raise ValueError("satd-16x32 cover %s not defined" % cover)
    import os

    _ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))
    p = os.path.join(_ROOT, "kernels", "satd-16x32", "candidates",
                     _FILES[cover])
    with open(p) as f:
        return f.read()
