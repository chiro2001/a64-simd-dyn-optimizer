...
def emit_vertical_fir(machine_ir, func_name, combo=None):
    ...
    if sliding == 2 and rows % 4 == 0:
        ...
        for k in range(11):
            lines.extend([
                ind + "svuint8_t rr%d = svld1_u8(pg16b," % k,
                ind + "    src + (%d) * srcStride%s);" % (k - 3, col),
        ...
        for g in range(rgroups):
            ...
            for o in range(4):
                lines.extend([
                    ...
                    ind + "    svst1_u8(pg16b, dst + %d * dstStride%s,"
        ...
        for r in range(rows):
            ...
            lines.extend([
                ind + "    int16x8_t lo = svget_neonq_s16(sum);",
                ind + "    int16x8_t hi = svget_neonq_s16(",
                ind + "        svtbl_s16(sum, hi8));",
                ind + "    uint8x16_t out = vcombine_u8(",
                ind + "        vqrshrun_n_s16(lo, 6),"
                " vqrshrun_n_s16(hi, 6));",
                ind + "    svst1_u8(pg16b, dst + %d * dstStride%s,"
                " svset_neonq_u8(svundef_u8(), out));" % (r, col),
                ind + "}",
            ])
    else:
        ...
        lines.append(ind + "{")
        lines.append(ind + "    svint16_t sum = svdup_n_s16(0);")
        lines.append(ind + "    for (int t = 0; t < 8; t++)")
        lines.append(ind + "    {")
        lines.append(ind + "        svint16_t w = svreinterpret_s16_u16(")
        lines.append(ind + "            svunpklo_u16(svld1_u8(pg16b,")
        lines.append(ind + "                src + (r - 3 + t) * srcStride"
                     "%s)));" % col)
        ...
        lines.extend([
            ...
            ind + "        svtbl_s16(sum, hi8));",
            ind + "    uint8x16_t out = vcombine_u8(",
            ind + "        vqrshrun_n_s16(lo, 6), vqrshrun_n_s16(hi, 6));",
            ind + "    svst1_u8(pg16b, dst + r * dstStride%s,"
            " svset_neonq_u8(svundef_u8(), out));" % col,
            ind + "}",
        ])
    if groups > 1:
        lines.append("    }")
