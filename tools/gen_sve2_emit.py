...
def emit_vertical_fir(machine_ir, func_name, combo=None):
    ...
    vertical taps), so one 16-lane s16 accumulator covers the whole row:
    sum += sign(c_t) * widen(row_{y-3+t}) * |c_t|, then saturating
    rounding narrow (vqrshrun, NEON bridge) and a 16-byte store.
    Coefficients come from g_lumaFilter phases 1..3 (generic source),
    with runtime coeffIdx dispatch.
    """
    nodes = machine_ir["nodes"]
    stores = [n for n in nodes if n.get("op") == "store"]
    rows = max(1, len(stores))
    sys.path.insert(0, os.path.join(ROOT, "tools"))
    from extract_x265_constants import parse_int16_tables  # noqa: E402
    cpp = os.path.join(ROOT, "third_party/x265/source/common/constants.cpp")
    tables = parse_int16_tables(open(cpp).read())
    ...
    if sliding == 2 and rows % 4 == 0:
        groups = rows // 4
        for k in range(11):
            lines.extend([
                "    svuint8_t rr%d = svld1_u8(pg16b," % k,
                "        src + (%d) * srcStride);" % (k - 3),
                "    svint16_t w%d = svreinterpret_s16_u16(" % k,
                "        svunpklo_u16(rr%d));" % k,
            ])
        for g in range(groups):
            lines.append("    {")
            for o in range(4):
                lines.append("        svint16_t sum%d = svdup_n_s16(0);"
                             % o)
                for t in range(8):
                    lines.extend([
                        "        int c%d_%d = cf[%d];" % (o, t, t),
                        "        if (c%d_%d > 0)" % (o, t),
                        "            sum%d = svmla_s16_x(p16, sum%d, w%d,"
                        " svdup_n_s16(c%d_%d));" % (o, o, o + t, o, t),
                        "        else if (c%d_%d < 0)" % (o, t),
                        "            sum%d = svmls_s16_x(p16, sum%d, w%d,"
                        " svdup_n_s16(-c%d_%d));" % (o, o, o + t, o, t),
                    ])
            for o in range(4):
                lines.extend([
                    "        int16x8_t lo%d = svget_neonq_s16(sum%d);"
                    % (o, o),
                    "        int16x8_t hi%d = svget_neonq_s16("
                    "svtbl_s16(sum%d, hi8));" % (o, o),
                    "        uint8x16_t out%d = vcombine_u8("
                    "vqrshrun_n_s16(lo%d, 6),"
                    " vqrshrun_n_s16(hi%d, 6));" % (o, o, o),
                    "        svst1_u8(pg16b, dst + %d * dstStride,"
                    " svset_neonq_u8(svundef_u8(), out%d));"
                    % (4 * g + o, o),
                ])
            if g < groups - 1:
                lines.extend([
                    "        rr0 = rr4; rr1 = rr5; rr2 = rr6;"
                ...
                for k in range(7, 11):
                    lines.extend([
                        "        rr%d = svld1_u8(pg16b,"
                        " src + (%d) * srcStride);" % (k, 4 * g + 8 + k - 7),
                        "        w%d = svreinterpret_s16_u16("
        ...
        for r in range(rows):
            lines.append("    {")
            lines.append("        svint16_t sum = svdup_n_s16(0);")
            for t in range(8):
                lines.extend([
                    "        int c%d = cf[%d];" % (t, t),
                    "        if (c%d > 0)" % t,
                    "            sum = svmla_s16_x(p16, sum, w%d,"
                    " svdup_n_s16(c%d));" % (r + t, t),
                    "        else if (c%d < 0)" % t,
                    "            sum = svmls_s16_x(p16, sum, w%d,"
                    " svdup_n_s16(-c%d));" % (r + t, t),
                ])
            lines.extend([
                ...
                "            vqrshrun_n_s16(lo, 6),"
                " vqrshrun_n_s16(hi, 6));",
                "        svst1_u8(pg16b, dst + %d * dstStride," % r,
        ...
        lines.extend([
            ...
            "                svunpklo_u16(svld1_u8(pg16b,",
            "                    src + (r - 3 + t) * srcStride)));",
            "            int c = cf[t];",
            "            if (c > 0)",
            "                sum = svmla_s16_x(p16, sum, w,",
            "                    svdup_n_s16(c));",
            "            else if (c < 0)",
            "                sum = svmls_s16_x(p16, sum, w,",
            "                    svdup_n_s16(-c));",
            "        }",
            "        int16x8_t lo = svget_neonq_s16(sum);",
            "        int16x8_t hi = svget_neonq_s16(",
