"""SVE2 SAO stats edge class 1 (vertical), 64x1, VL=256 (docs/45).

E1 is simpler than E0: no horizontal carry. upBuff1 holds -sign_up
(negated in place on entry), edge_type = sign_down - sign_up, and the
buffer is updated to sign_down for the next row.

Two structure families (same as Stats E0):
...
def _sign16(a_expr, b_expr):
    # sign(a-b) = (b>a ? -1 : 0) - (a>b ? -1 : 0); svcmpgt returns a
    # predicate, svsel turns it into 0/-1.
...
def emit_block16(func_name="dynopt_sao_stats_e1_64_sve2"):
    lines = _emit_core16()
    lines.append(
        "extern \"C\" void %s(const int16_t* diff, const uint8_t* rec,"
        " intptr_t stride, int8_t* upBuff1, int32_t* stats,"
        " int32_t* count)" % func_name)
    lines.append("{")
    lines.extend([
    ...
    for x in range(0, 64, 16):
        lines.extend([
            ...
            "        int8x16_t sd = sign16n(rec + %d, rec + %d + stride);"
            % (x, x),
            "        int8x16_t et = vsubq_s8(sd, su);",
            "        vst1q_s8(upBuff1 + %d, sd);" % x,
...
def emit_block32(func_name="dynopt_sao_stats_e1_64_sve2"):
    lines = [
        "#include <arm_sve.h>",
        "#include <stdint.h>",
        "#include <stddef.h>",
        ...
        "        svuint8_t nxt = svld1_u8(pg8, rec + x + stride);",
        "        svint8_t sd = %s;" % _sign16("cur", "nxt"),
        "        svint8_t et = svsub_s8_x(pg8, sd, su);",
        "        svst1_s8(pg8, upBuff1 + x, sd);",
