"""Inlined psyCost_pp candidates (GCC12 calc_energy_8x8 outline fix).

Upstream psyCost_pp_neon calls calc_energy_8x8 twice per 8x8 block
through a `bl` (outlined by GCC12, ~2 calls per block; psyCost is
~3% of the 1080p encode). This emitter reproduces the upstream dataflow
with all hadamard helpers forced inline; semantics bit-exact.
"""
...
def emit(size, func_name):
    """size = 1..4 -> 8x8 .. 64x64 psy_cost_pp candidate."""
    dim = 1 << (size + 2)
    return (HELPERS + """
extern "C" int %s(const uint8_t* source, intptr_t sstride,
