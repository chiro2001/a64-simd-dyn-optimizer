#!/usr/bin/env python3
"""SVE2 (VL=256) emitter for the 8-bit horizontal 8-tap luma filter
(interp8, 8x8), upstream-exact (path A: sdot .d slices + NEON-bridge
saturating narrow).

Semantics (x265 interp8_horiz_pp_neon): src -= 3; out[c] =
sat_u8(round(SUM_{k=0..7} coeff[phase][k] * src[c-3+k] / 2^6)).

Structure (mirrors the DCT32 v3 lane-per-output trick):
  - one 16-byte window W = src[row]-3 .. +12 (svld1ub_u16, widening);
  - three TBL slices (windows overlap, so X1 is reused):
      X0 = outs0-3 taps0-3, X1 = outs0-3 taps4-7 == outs4-7 taps0-3,
      X2 = outs4-7 taps4-7;
  - per half: 2 SDOT .d (4 outputs accumulate their full 8 taps in 4
    s64 lanes) + uzp1_s32 + rshrnb(6) + uzp1_s16 + vqmovun (NEON bridge,
    saturating) + 4-lane predicate store.
"""

import argparse


COEFFS = {
    1: [-1, 4, -10, 58, 17, -5, 1, 0],
    2: [-1, 4, -11, 40, 40, -11, 4, -1],
    3: [0, 1, -5, 17, 58, -10, 4, -1],
}


def cpp_constants():
    lines = ["static const int16_t CTBL[4][2][16] = {"]
    for ph in (1, 2, 3):
        c = COEFFS[ph]
        lines.append("    {")
        for m in range(2):
            g = c[4 * m:4 * m + 4]
            body = ", ".join(str(x) for x in g)
            lines.append("        { %s, %s, %s, %s },  // m=%d"
                         % (body, body, body, body, m))
        lines.append("    },  // phase %d" % ph)
    lines.append("};")
    lines.append("""\
static const uint16_t IDX_X0[16] =
    { 0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 3, 4, 5, 6 };
static const uint16_t IDX_X1[16] =
    { 4, 5, 6, 7, 5, 6, 7, 8, 6, 7, 8, 9, 7, 8, 9, 10 };
static const uint16_t IDX_X2[16] =
    { 8, 9, 10, 11, 9, 10, 11, 12, 10, 11, 12, 13, 11, 12, 13, 14 };
static const uint32_t IDX_08[16] =
    { 0, 1, 2, 3, 8, 9, 10, 11, 0, 1, 2, 3, 8, 9, 10, 11 };
""")
    return "\n".join(lines)


def emit(func_name="dynopt_interp8_8x8_sve2"):
    return """\
// Generated by tools/emit_interp8_sve2_shared.py -- do not edit by hand.
// 8-bit horizontal 8-tap luma filter 8x8 (SVE2, VL=256), upstream-exact.
#include <arm_sve.h>
#include <arm_neon.h>
#include <arm_neon_sve_bridge.h>

%s

extern "C" void %s(const uint8_t* src, intptr_t srcStride,
                   uint8_t* dst, intptr_t dstStride, int coeffIdx)
{
    const svbool_t p16 = svptrue_b16();
    const svint64_t zero64 = svdup_n_s64(0);
    const int ph = (coeffIdx >= 1 && coeffIdx <= 3) ? coeffIdx : 2;
    const svint16_t c0 = svld1_s16(p16, CTBL[ph - 1][0]);
    const svint16_t c1 = svld1_s16(p16, CTBL[ph - 1][1]);
    const svuint16_t ix0 = svld1_u16(p16, IDX_X0);
    const svuint16_t ix1 = svld1_u16(p16, IDX_X1);
    const svuint16_t ix2 = svld1_u16(p16, IDX_X2);
    const svuint32_t ix08 = svld1_u32(svptrue_b32(), IDX_08);

    for (int r = 0; r < 8; r++)
    {
        const uint8_t* row = src + r * srcStride - 3;
        svuint16_t W = svld1ub_u16(p16, row);
        svint16_t X0 = svreinterpret_s16_u16(svtbl_u16(W, ix0));
        svint16_t X1 = svreinterpret_s16_u16(svtbl_u16(W, ix1));
        svint16_t X2 = svreinterpret_s16_u16(svtbl_u16(W, ix2));

        svint64_t t0 = svdot_s64(zero64, X0, c0);
        t0 = svdot_s64(t0, X1, c1);
        svint32_t lo0 = svuzp1_s32(svreinterpret_s32_s64(t0),
                                   svreinterpret_s32_s64(t0));
        svint64_t t1 = svdot_s64(zero64, X1, c0);
        t1 = svdot_s64(t1, X2, c1);
        svint32_t lo1 = svuzp1_s32(svreinterpret_s32_s64(t1),
                                   svreinterpret_s32_s64(t1));
        svint32_t all = svtbl2_s32(svcreate2_s32(lo0, lo1), ix08);
        svint16_t rr = svrshrnb_n_s32(all, 6);
        svint16_t rz = svuzp1_s16(rr, rr);
        uint8x8_t u = vqmovun_s16(svget_neonq_s16(rz));
        // Exactly 8 bytes per row: a full-width SVE store (or the 16-byte
        // NEON bridge value) would scribble past the 8x8 tile and fail the
        // IPFilterHarness whole-buffer memcmp gate.
        vst1_u8(dst + r * dstStride, u);
    }
}
""" % (cpp_constants(), func_name)


def emit_sdot_h(func_name=None, width=8, height=8, unroll=False,
                pairsum="addp"):
    """Path B (docs/22 §5, SVE2p3): sdot z.h,z.b,z.b direct 8-tap
    horizontal filter. 16 h-lanes = 8 pixels x 2 tap-pairs
    ((0,1)+(4,5) in lane 2p, (2,3)+(6,7) in lane 2p+1), addp sums
    pairs, svqrshrunb rounds/shifts/clamps to u8. Needs SVE2p3 QEMU
    (custom build, patches/qemu-sve2p3-sdot-btoh.patch).
    width/height: 8-pixel units per row; every third unit starts a new
    32-byte window at row+24 (units 0-2 share row, 3-5 share row+24,
    6-8 share row+48).  pairsum: "addp" (SVE2 addp(t,t)+uzp1, docs/22
    §5.7, 1 instr/unit saved) or "uzp" (uzp1+uzp2+add baseline)."""
    if width % 8:
        raise ValueError("emit_sdot_h: width must be a multiple of 8")
    if func_name is None:
        func_name = "dynopt_interp8_%dx%d_sve2_sdoth" % (width, height)
    units = width // 8
    n_windows = (units + 2) // 3
    needed_offsets = sorted({(u % 3) * 8 for u in range(units)})
    ix_loads = []
    if 0 in needed_offsets:
        ix_loads += [
            "    const svuint8_t ix0 = svld1_u8(p32, IX0B);",
            "    const svuint8_t ix1 = svld1_u8(p32, IX1B);",
        ]
    if 8 in needed_offsets:
        ix_loads += [
            "    const svuint8_t ix0_8 = svld1_u8(p32, IX0B8);",
            "    const svuint8_t ix1_8 = svld1_u8(p32, IX1B8);",
        ]
    if 16 in needed_offsets:
        ix_loads += [
            "    const svuint8_t ix0_16 = svld1_u8(p32, IX0B16);",
            "    const svuint8_t ix1_16 = svld1_u8(p32, IX1B16);",
        ]
    window_lines = []
    for q in range(n_windows):
        addr = "row" if q == 0 else "row + %d" % (24 * q)
        window_lines.append(
            "        svuint8_t W32_%d = svld1_u8(p32, %s);" % (q, addr))
    for q in range(n_windows):
        window_lines.append(
            "        svint8_t W_%d = svreinterpret_s8_u8(" % q)
        window_lines.append(
            "            svsub_n_u8_x(svptrue_b8(), W32_%d, 128));" % q)
    unit_calls = []
    for u in range(units):
        q = u // 3
        o = (u % 3) * 8
        ix0name = "ix0" if o == 0 else ("ix0_8" if o == 8 else "ix0_16")
        ix1name = "ix1" if o == 0 else ("ix1_8" if o == 8 else "ix1_16")
        unit_calls.append(
            "        interp8_unit8(W_%d, %s, %s, c0, c1, off4096,"
            " drow + %d, p8h8);" % (q, ix0name, ix1name, 8 * u))
    head = """\
// Generated by tools/emit_interp8_sve2_shared.py -- do not edit by hand.
// 8-bit horizontal 8-tap luma filter %(width)dx%(height)d (SVE2p3,
// VL=256), path B.
#include <arm_sve.h>

static const int8_t CB0[32] =
    { -1, 4, -10, 58, -1, 4, -10, 58, -1, 4, -10, 58, -1, 4, -10, 58,
      -1, 4, -10, 58, -1, 4, -10, 58, -1, 4, -10, 58, -1, 4, -10, 58 };
static const int8_t CB1[32] =
    { 17, -5, 1, 0, 17, -5, 1, 0, 17, -5, 1, 0, 17, -5, 1, 0,
      17, -5, 1, 0, 17, -5, 1, 0, 17, -5, 1, 0, 17, -5, 1, 0 };
static const int8_t CB2[32] =
    { -1, 4, -11, 40, -1, 4, -11, 40, -1, 4, -11, 40, -1, 4, -11, 40,
      -1, 4, -11, 40, -1, 4, -11, 40, -1, 4, -11, 40, -1, 4, -11, 40 };
static const int8_t CB3[32] =
    { 40, -11, 4, -1, 40, -11, 4, -1, 40, -11, 4, -1, 40, -11, 4, -1,
      40, -11, 4, -1, 40, -11, 4, -1, 40, -11, 4, -1, 40, -11, 4, -1 };
static const int8_t CB4[32] =
    { 0, 1, -5, 17, 0, 1, -5, 17, 0, 1, -5, 17, 0, 1, -5, 17,
      0, 1, -5, 17, 0, 1, -5, 17, 0, 1, -5, 17, 0, 1, -5, 17 };
static const int8_t CB5[32] =
    { 58, -10, 4, -1, 58, -10, 4, -1, 58, -10, 4, -1, 58, -10, 4, -1,
      58, -10, 4, -1, 58, -10, 4, -1, 58, -10, 4, -1, 58, -10, 4, -1 };
static const uint8_t IX0B[32] =
    { 0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 3, 4, 5, 6,
      4, 5, 6, 7, 5, 6, 7, 8, 6, 7, 8, 9, 7, 8, 9, 10 };
static const uint8_t IX1B[32] =
    { 4, 5, 6, 7, 5, 6, 7, 8, 6, 7, 8, 9, 7, 8, 9, 10,
      8, 9, 10, 11, 9, 10, 11, 12, 10, 11, 12, 13, 11, 12, 13, 14 };
static const uint8_t IX0B8[32] =
    { 8, 9, 10, 11, 9, 10, 11, 12, 10, 11, 12, 13, 11, 12, 13, 14,
      12, 13, 14, 15, 13, 14, 15, 16, 14, 15, 16, 17, 15, 16, 17, 18 };
static const uint8_t IX1B8[32] =
    { 12, 13, 14, 15, 13, 14, 15, 16, 14, 15, 16, 17, 15, 16, 17, 18,
      16, 17, 18, 19, 17, 18, 19, 20, 18, 19, 20, 21, 19, 20, 21, 22 };
static const uint8_t IX0B16[32] =
    { 16, 17, 18, 19, 17, 18, 19, 20, 18, 19, 20, 21, 19, 20, 21, 22,
      20, 21, 22, 23, 21, 22, 23, 24, 22, 23, 24, 25, 23, 24, 25, 26 };
static const uint8_t IX1B16[32] =
    { 20, 21, 22, 23, 21, 22, 23, 24, 22, 23, 24, 25, 23, 24, 25, 26,
      24, 25, 26, 27, 25, 26, 27, 28, 26, 27, 28, 29, 27, 28, 29, 30 };

static inline svint16_t sdot_h(svint16_t acc, svint8_t a, svint8_t b)
{
    asm volatile("sdot %%0.h, %%1.b, %%2.b"
                 : "+w"(acc) : "w"(a), "w"(b));
    return acc;
}

static inline svint16_t sdot_h_acc(svint16_t acc, svint8_t a, svint8_t b)
{
    // MOVPRFX (unpredicated) is fused with the following SVE instruction on
    // real pipes, so it costs no issue slot; it both resets the accumulator
    // and folds in the +8192 DC offset (docs/22 offset trick).
    // MOVPRFX requires Zd != Zn and the following sdot must not use Zd as a
    // non-destructive source: early-clobber on the output forces a fresh
    // register distinct from acc/a/b.
    svint16_t out;
    asm volatile("movprfx %%0, %%1\\n\\tsdot %%0.h, %%2.b, %%3.b"
                 : "=&w"(out) : "w"(acc), "w"(a), "w"(b));
    return out;
}

static inline svint16_t addp_h(svint16_t a, svbool_t pg)
{
    // SVE2 ADDP (predicated): even slot i = a[i]+a[i+1], odd slot i =
    // a[i-1]+a[i]; with vn==vm==a this yields [p0,p0,p1,p1,...] for the
    // pair-sum t, and uzp1 compacts to [p0..p7] (1 instr saved per unit
    // vs uzp1+uzp2+add; verified 2026-08-14).
    asm volatile("addp %%0.h, %%1/m, %%0.h, %%0.h"
                 : "+w"(a) : "Upl"(pg));
    return a;
}

static inline void interp8_unit8(
    svint8_t W, svuint8_t ix0, svuint8_t ix1,
    svint8_t c0, svint8_t c1, svint16_t off4096,
    uint8_t* dstp, svbool_t p8h8)
{
    svint8_t X0 = svtbl_s8(W, ix0);
    svint8_t X1 = svtbl_s8(W, ix1);
    svint16_t t = sdot_h_acc(off4096, X0, c0);
    t = sdot_h(t, X1, c1);
    // lane 2p = pixel p taps (0,1)+(4,5); lane 2p+1 = taps (2,3)+(6,7).
    %(pairsum)s
    svuint8_t u = svqrshrunb_n_s16(pixels, 6);
    // SVE2 bottom-narrow writes each result to the low byte of its
    // 16-bit slot (bytes 0,2,4,...); uzp1 compacts to bytes 0..7.
    svuint8_t uz = svuzp1_u8(u, u);
    svst1_u8(p8h8, dstp, uz);
}

extern "C" void %(func_name)s(const uint8_t* src, intptr_t srcStride,
                   uint8_t* dst, intptr_t dstStride, int coeffIdx)
{
    const svbool_t p32 = svptrue_b8();
    const svbool_t p8h8 = svwhilelt_b8((uint32_t)0, (uint32_t)8);
    const int ph = (coeffIdx >= 1 && coeffIdx <= 3) ? coeffIdx : 2;
    const svint8_t c0 = svld1_s8(p32, (const int8_t*)
        (ph == 1 ? CB0 : (ph == 3 ? CB4 : CB2)));
    const svint8_t c1 = svld1_s8(p32, (const int8_t*)
        (ph == 1 ? CB1 : (ph == 3 ? CB5 : CB3)));
%(ix_loads)s
    // Each output pixel = t[2p] + t[2p+1]; the 8192 DC offset must be
    // split across both accumulator lanes (4096 each) so the pair-sum
    // restores the full +8192 once (verified 2026-08-14).
    const svint16_t off4096 = svdup_n_s16(4096);

%(unroll_pragma)s    for (int r = 0; r < %(height)d; r++)
    {
        const uint8_t* row = src + r * srcStride - 3;
%(window_loads)s
        uint8_t* drow = dst + r * dstStride;
%(unit_calls)s
    }
}
""" % {"func_name": func_name, "width": width, "height": height,
       "ix_loads": "\n".join(ix_loads),
       "window_loads": "\n".join(window_lines),
       "unit_calls": "\n".join(unit_calls),
       "unroll_pragma": ("#pragma clang loop unroll(full)\n"
                         if unroll else ""),
       "pairsum": ("svint16_t d = addp_h(t, svptrue_b16());\n    "
                   "svint16_t pixels = svuzp1_s16(d, d);"
                   if pairsum == "addp" else
                   "svint16_t pixels = svadd_s16_x(\n"
                   "        svptrue_b16(), svuzp1_s16(t, t), "
                   "svuzp2_s16(t, t));")}
    return head


def emit_vpp(func_name, width=16, height=16, acc_split=1, tile=None):
    """Vertical 8-tap luma filter (SVE2, VL=256), square WxH.

    Upstream interp8_vert_pp_neon<8,W,H> contract: src is the block
    top-left; output row r = sat_u8(round(SUM f_k * src[(r-3+k)*stride +
    c] / 2^6)).  Each 16-pixel unit is one z.h vector (16 lanes).

    Sliding-row pipeline (2026-08-14): (H+7) row-units (rows -3..H+3)
    are loaded once and stay in registers; output row i consumes
    v_i..v_{i+7} with coefficients c0..c7.  Per unit 8 MLA + pair adds +
    narrow; the (b-128) bias keeps s16 accumulators in range (8192 split
    across accumulators).  acc_split: 1 = two 4-product accumulators
    (fewest instructions, 4-deep MLA chain); 2 = four 2-product
    accumulators (2-deep chains, more ILP, +2 adds/unit/row).
    """
    units = max(1, (width + 15) // 16)
    rows = height
    n_rows = rows + 7
    coeff = {
        1: [-1, 4, -10, 58, 17, -5, 1, 0],
        2: [-1, 4, -11, 40, 40, -11, 4, -1],
        3: [0, 1, -5, 17, 58, -10, 4, -1],
    }
    lines = ["// Generated by tools/emit_interp8_sve2_shared.py -- do not",
             "// edit by hand. Vertical 8-tap luma %dx%d (SVE2, VL=256)."
             % (width, height),
             "#include <arm_sve.h>", ""]
    lines.append("static inline svint16_t vrow(const uint8_t* p, "
                 "intptr_t st, int k, int u, svbool_t p16)")
    lines.append("{")
    lines.append("    svuint16_t w = svld1ub_u16(p16, "
                 "p + k * st + u * 16);")
    lines.append("    // (b - 128) keeps 4-product s16 sums in range.")
    lines.append("    return svreinterpret_s16_u16("
                 "svsub_n_u16_x(p16, w, 128));")
    lines.append("}")
    lines.append("")
    lines.append("extern \"C\" void %s(const uint8_t* src, "
                 "intptr_t srcStride," % func_name)
    lines.append("                   uint8_t* dst, intptr_t dstStride, "
                 "int coeffIdx)")
    lines.append("{")
    lines.append("    const svbool_t p16 = svptrue_b16();")
    lines.append("    const svbool_t p8b = svwhilelt_b8((uint32_t)0, "
                 "(uint32_t)16);")
    for u in range(units):
        lines.append("    const svbool_t pout%d = svwhilelt_b8("
                     "(uint32_t)0, (uint32_t)%d);"
                     % (u, min(16, width - u * 16)))
    lines.append("    const int ph = (coeffIdx >= 1 && coeffIdx <= 3) "
                 "? coeffIdx : 2;")
    for k in range(8):
        lines.append("    const svint16_t c%d = svdup_n_s16("
                     "ph == 1 ? %d : (ph == 3 ? %d : %d));"
                     % (k, coeff[1][k], coeff[3][k], coeff[2][k]))
    n_accs = 2 if acc_split == 1 else 4
    lines.append("    // DC offset 8192 split across the accumulators")
    lines.append("    // (%d accs -> %d each)."
                 % (n_accs, 8192 // n_accs))
    lines.append("    const svint16_t off = svdup_n_s16(%d);"
                 % (8192 // n_accs))

    def emit_body(rows, t0, ind):
        n_rows = rows + 7
        out = []
        out.append("%sconst uint8_t* rb = src + %s * srcStride"
                   " - 3 * srcStride;" % (ind, t0))
        for i in range(min(8, n_rows)):
            for u in range(units):
                out.append("%ssvint16_t v%d_%d = vrow(rb, srcStride,"
                           " %d, %d, p16);" % (ind, i, u, i, u))
        out.append("")
        for i in range(rows):
            if i + 8 < n_rows:
                for u in range(units):
                    out.append("%ssvint16_t v%d_%d = vrow(rb, srcStride,"
                               " %d, %d, p16);" % (ind, i + 8, u,
                                                   i + 8, u))
            for u in range(units):
                out.append("%s{" % ind)
                if acc_split == 1:
                    accs = [("a0", (0, 2, 4, 6)), ("a1", (1, 3, 5, 7))]
                else:
                    accs = [("a0", (0, 4)), ("a1", (1, 5)),
                            ("a2", (2, 6)), ("a3", (3, 7))]
                for name, taps in accs:
                    out.append("%s    svint16_t %s = off;" % (ind, name))
                    for k in taps:
                        out.append("%s    %s = svmla_s16_x(p16, %s,"
                                   " v%d_%d, c%d);"
                                   % (ind, name, name, i + k, u, k))
                out.append("%s    svint16_t pixels = %s;"
                           % (ind, accs[0][0]))
                for name, _ in accs[1:]:
                    out.append("%s    pixels = svadd_s16_x(p16, pixels,"
                               " %s);" % (ind, name))
                out.append("%s    svuint8_t u8 = svqrshrunb_n_s16("
                           "pixels, 6);" % ind)
                out.append("%s    svuint8_t uz = svuzp1_u8(u8, u8);" % ind)
                out.append("%s    svst1_u8(pout%d, dst + (%s + %d) *"
                           " dstStride + %d, uz);"
                           % (ind, u, t0, i, u * 16))
                out.append("%s}" % ind)
        return out

    if tile and tile < height:
        if height % tile:
            raise ValueError("emit_vpp tile must divide height")
        lines.append("    for (int t0 = 0; t0 < %d; t0 += %d)"
                     % (height, tile))
        lines.append("    {")
        lines.extend(emit_body(tile, "t0", "        "))
        lines.append("    }")
    else:
        lines.extend(emit_body(height, "0", "    "))
    lines.append("}")
    return "\n".join(lines)


def emit_vpp_16x16(func_name="dynopt_interp8_16x16_sve2_vpp",
                   acc_split=1):
    return emit_vpp(func_name, width=16, height=16, acc_split=acc_split)


def emit_vpp_32x32(func_name="dynopt_interp8_32x32_sve2_vpp",
                   acc_split=1):
    return emit_vpp(func_name, width=32, height=32, acc_split=acc_split)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("out", default="generated/interp8/sve2_8x8.cpp")
    ap.add_argument("--compute", default="sdot-d",
                    choices=("sdot-d", "sdot-h"))
    ap.add_argument("--n", type=int, default=8, choices=(8, 16, 32),
                    help="square shape (8/16/32; sdot-h only, docs/22 §5.5)")
    ap.add_argument("--pairsum", default="addp", choices=("addp", "uzp"),
                    help="pair-sum scheme (addp=docs/22 §5.7, uzp=baseline; "
                         "sve1 substitution must use uzp)")
    args = ap.parse_args()
    with open(args.out, "w") as f:
        if args.compute == "sdot-h":
            f.write(emit_sdot_h(width=args.n, height=args.n,
                                pairsum=args.pairsum))
        else:
            f.write(emit())
    print("wrote %s" % args.out)


if __name__ == "__main__":
    main()
