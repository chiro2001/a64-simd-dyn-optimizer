...
def emit_vpp_16x16(func_name="dynopt_interp8_16x16_sve2_vpp",
                   acc_split=1):
    """Vertical 8-tap luma filter 16x16 (SVE2, VL=256).

    Upstream interp8_vert_pp_neon<8,16,16> contract: src is the block
    top-left; output row r = sat_u8(round(SUM f_k * src[(r-3+k)*stride +
    c] / 2^6)).  Each 16-pixel row is one z.h vector (16 lanes).

    Sliding-row pipeline (2026-08-14): 23 row vectors v0..v22 (rows -3..
    +19) are loaded once and stay in registers; output row i consumes
    v_i..v_{i+7} with coefficients c0..c7.  Per row 8 MLA + pair adds +
    narrow; the (b-128) bias keeps s16 accumulators in range (4096 per
    accumulator, sums restore 8192 once).  acc_split: 1 = two 4-product
    accumulators (fewest instructions, 4-deep MLA chain); 2 = four
    2-product accumulators (2-deep chains, more ILP, +2 adds/row).
    """
    rows = 16
    n_rows = rows + 7  # 23 loaded rows
    coeff = {
        1: [-1, 4, -10, 58, 17, -5, 1, 0],
        2: [-1, 4, -11, 40, 40, -11, 4, -1],
        3: [0, 1, -5, 17, 58, -10, 4, -1],
    ...
    lines.append("static inline svint16_t vrow(const uint8_t* p, "
                 "intptr_t st, int k, svbool_t p16)")
    lines.append("{")
    lines.append("    svuint16_t u = svld1ub_u16(p16, p + k * st);")
    lines.append("    // (b - 128) keeps 4-product s16 sums in range.")
    lines.append("    return svreinterpret_s16_u16("
                 "svsub_n_u16_x(p16, u, 128));")
    lines.append("}")
    lines.append("")
    ...
    for i in range(n_rows):
        lines.append("    svint16_t v%d = vrow(rb, srcStride, %d, p16);"
                     % (i, i))
    lines.append("")
    for i in range(rows):
        lines.append("    {")
        ...
        for name, taps in accs:
            ...
            for k in taps:
                lines.append("        %s = svmla_s16_x(p16, %s, v%d, c%d);"
                             % (name, name, i + k, k))
        lines.append("        svint16_t pixels = %s;" % accs[0][0])
        for name, _ in accs[1:]:
            lines.append("        pixels = svadd_s16_x(p16, pixels, %s);"
                         % name)
        lines.append("        svuint8_t u = svqrshrunb_n_s16(pixels, 6);")
        lines.append("        svuint8_t uz = svuzp1_u8(u, u);")
        lines.append("        svst1_u8(p8b, dst + %d * dstStride, uz);"
                     % i)
        lines.append("    }")
    lines.append("}")
    return "\n".join(lines)
