"""SVE2 SAO stats edge class 3 (diagonal 45deg), 64x1, VL=256
(docs/45). E3 is E1 with the down-left neighbor: upBuff1 holds
-sign_up (negated on entry), sign_down = sign(rec[x]-rec[x+stride-1]),
edge_type = sign_down - sign_up, upBuff1[x-1] = sign_down, tail
upBuff1[63] = sign(rec[64]-rec[63+stride]).

Two structure families (same as Stats E0/E1/E2).
"""
...
def emit_block16(func_name="dynopt_sao_stats_e3_64_sve2"):
    lines = [
        "#include <arm_neon.h>",
        "#include <arm_sve.h>",
...
def emit_block32(func_name="dynopt_sao_stats_e3_64_sve2"):
    lines = [
        "#include <arm_sve.h>",
        "#include <stdint.h>",
        "#include <stddef.h>",
