#!/usr/bin/env python3
"""dct32 dual-group 16-lane SVE emitter validation:
1) generated object has zero NEON registers (--no-neon);
2) numeric equality vs the 8-lane pure-SVE dct32 (runtime guard
   ...
   deterministic input, then the dumps are compared."""
...
from dct32_dual_sve_emit import emit_dual_sve  # noqa: E402
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < 40; it++)
    {
        fill(src, it < 5 ? it : 0);
        if (mode == 0)
            dynopt_dct32_sve8_anyvl(src, out, 32);
        else
            dynopt_dct32_sve16(src, out, 32);
        for (int i = 0; i < 32 * 32; i++)
            printf("%d%c", out[i], (i & 31) == 31 ? '\n' : ' ');
    }
    return 0;
}
"""
...
class Dct32Sve16Test(unittest.TestCase):
    ...
    def test_sve16_no_neon(self):
        src = self.write("tmp-dual-dct32.cpp", emit_dual_sve())
        obj = os.path.join(ROOT, "build", "tmp-dual-dct32.o")
        self.build(src, obj)
        g = subprocess.run(
    ...
    def test_sve16_vs_sve8_vq1(self):
        pure = emit_pure_sve("dynopt_dct32_sve8_anyvl")
        pure = pure.replace("    if (svcntb() != 16) return;\n", "")
        src = self.write("tmp-sve8-anyvl-dct32.cpp", pure)
        obj = os.path.join(ROOT, "build", "tmp-sve8-anyvl-dct32.o")
        self.build(src, obj)
        dual_src = self.write("tmp-dual-dct32.cpp", emit_dual_sve())
        dual_obj = os.path.join(ROOT, "build", "tmp-dual-dct32.o")
        self.build(dual_src, dual_obj)
        drv = self.write("tmp-dct32-sve16-driver.cpp", DRIVER)
        binp = os.path.join(ROOT, "build", "tmp-dct32-sve16-driver")
        r = subprocess.run(
        ...
        self.assertEqual(len(rv), 40 * 32 * 32)
