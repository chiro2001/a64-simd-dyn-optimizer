...
searches:
  ...
  shape  full     -- 32 fully-unrolled 8-element blocks
         loop     -- one 8-element body in a for loop (smaller code,
                    less register pressure; compiler schedules ILP)
...
def emit_256(func_name="dynopt_quant_256_sve2", count="vaddv",
    ...
    int32x4_t %(p)sd1 = vshlq_s32(vsubq_s32(%(p)sp1,
                             ...
                             qbits8_neg);
    vst1q_s32(du + %(idx)s, %(p)sd0);
    vst1q_s32(du + %(idx)s + 4, %(p)sd1);
...
"""
    if shape == "pair":
        ...
        for gi in range(32):
            steps.append(step % {
                "idx": str(8 * gi), "p": "u%d_" % gi,
                "nz_step": nz_step % {"p": "u%d_" % gi}})
        steps_src = "\n".join(steps)
    else:
        steps_src = ("    for (int i = 0; i < 256; i += 8)\n"
                     "    {\n"
                     + "\n".join(
                         "    " + ln for ln in
                         (step % {"idx": "i", "p": "",
...
// quant 256 pure NEON, count=%(count)s, shape=%(shape)s.
#include <arm_neon.h>
#include <stdint.h>
...
""" % {"count": count, "shape": shape, "func": func_name,
       "nz_src": nz_src, "steps": steps_src, "nz_end": nz_end}


def emit_combo(combo):
    return emit_256(
        count=combo.get("count", "vaddv"),
        shape=combo.get("shape", "full"))
