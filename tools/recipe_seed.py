#!/usr/bin/env python3
"""Generic seed analysis: MachineIR 结构检测 + 族识别 + 配方轴种子。

输入：任意 seed 的 machine-ir.json（原名 dct16_recipe_seed.py，已泛化，
docs/40；dct16 仍是首个验收样例）。
...
输出：recipe-seed.json
  - family_hint：由引用的常量符号/指令形态推断结构族
    （butterfly / fir / fir-vertical / diff-sum）
...
def main():
    ...
    ap.add_argument("--kernel", default="unknown",
                    help="kernel name for the report (default: unknown)")
