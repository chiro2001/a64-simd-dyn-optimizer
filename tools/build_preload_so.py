...
def candidate_sources(kernel, isa, vl=None):
    ...
    if kernel == "dct16" and isa == "sve2" and (vl is None or vl == 32):
        if os.environ.get("AGO_IR_SVE16") == "1":
            ...
            if os.path.exists(p):
                return [p]
        if os.environ.get("AGO_WIDE_SVE2") == "1":
            # Width-native SVE2 emitter candidate (VL=256):
            # neon_bridge mode reproduces op895 bit-exact with
            # permute_depth_ratio=12.0% (better than op895's 18.5%).
            # Generated by optimizer/ir/dct16_wide_sve2.py.
            p = os.path.join(d, "best_wide_sve2_neon_bridge.cpp")
            if os.path.exists(p):
                return [p]
    ...
    if kernel == "dct32" and isa == "sve2" and (vl is None or vl == 32):
        if os.environ.get("AGO_IR_SVE16") == "1":
            ...
            if os.path.exists(p):
                return [p]
        if os.environ.get("AGO_WIDE_SVE2") == "1":
            # Loop-based k-sections emitter (VL=256):
            # Same algorithm as opbase, loop-ified k-sections for
            # compiler scheduling freedom. permute_depth_ratio=19.4%
            # vs opbase 21.2%, cp_lat=75 vs 123. Bit-exact QEMU 200iters.
            # Generated by optimizer/ir/dct32_wide_sve2.py.
            p = os.path.join(d, "best_wide_sve2_loop.cpp")
            if os.path.exists(p):
                return [p]
    ...
    if kernel == "cost-coeff-nxn":
        ...
        if os.path.exists(p):
            return [p]
    if kernel.startswith("interp8") and isa == "sve2" and \
            (vl is None or vl == 32) and \
            os.environ.get("AGO_WIDE_SVE2") == "1" and \
            _interp_field(kernel) == "luma_hpp":
        # Width-native svdot_s32 (s8xs8->s32) interp8 hpp candidate
        # (VL=256): 8 pixel/lane direct output, no addp RMW.
        # permute_depth_ratio 20.5% (8x8) vs best_sve2 53.3%.
        # Bit-exact vs NEON ref QEMU 200x6x3. Generated by
        # optimizer/ir/interp8_wide_sve2.py.
        shape = _shape_of(kernel)
        if shape:
            w, h = shape
            if w == 8 and h == 8:
                fname = "best_wide_sve2_svdot32.cpp"
            else:
                fname = "best_wide_sve2_svdot32_%dx%d.cpp" % (w, h)
