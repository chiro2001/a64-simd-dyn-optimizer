...
> **2026-08-14 实机首轮 cycles（内部保密机型，型号/版本链不记录）**：
> 该机管道配置为 **NEON 4×128 / SVE2 2×256**（与 960 目标的 SVE
> 4×256 不同，仅作成本模型校准，不改变 960 验收）。同机 dct16：
> NEON 基线 174 cycles、内部算子 204、本项目 upstream-exact 213、
> legacy-internal-exact 209。结论：a) **指令数代理（fused_uop：我们
> 704 vs 内部 827）未转化为周期优势**（209 vs 204），代理对跨实现
> 周期不具备预测性，周期差异来自依赖链/发射/访存等微观因素；
> b) SVE2 两候选均慢于同机 NEON（174），同算力宽度下 +130% 目标
> 尚未达成；c) scatter 不是主因（我们 sg=4 < 内部 32，仍更慢）。
