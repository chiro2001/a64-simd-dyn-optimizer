...
> **2026-08-13 实际流程优先说明（保留本文原设计）**
> 本文件是最初的“里程碑式”设计，仍保留作为长期路线参考。但当前项目
> **优先按实际执行流程推进**：以“假设 → 最小探针 → 实现 → 门禁 →
> 提交/回滚 → 下一假设”的快速实证循环为主，里程碑作为硬性验收而非
> 排期计划。实际流程的完整描述、输入/输出、Agent 依赖点与强化路线见
> `docs/23-current-flow-and-agent-deps.md`；优先顺序为：
> 1. 正确性/门禁/计数全部自动化（上游 oracle + TestBenchLite + full-call
>    fused_uop）；
> 2. 结构发现由 op 级原子 rewrite + 序列搜索驱动（不再只靠手写轴）；
> 3. 咨询降频后台化（每 3 个阶段 1 次，GPT-5.6-sol，只写固定目录）；
> 4. 负结果一律提交/回滚并归档，避免重复踩坑；
> 5. 实机 paired cycles（920B/G/960）是唯一性能结论来源，指令数只是代理。
