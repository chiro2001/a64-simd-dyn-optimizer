...
## 12. 2026-08-14 Step 2 进度：线性发射器已修爆炸，语义 bug 定位中

- 指数爆炸已修复：改为**线性 goto 发射**（每块一次、全变量预声明、
  phi 在边赋值）——idct16 6727 行 codegen 0.00s / 22MB RSS（原 16GB）；
- 入口块发现：LLVM IR printer 省略入口块标签行，但 phi 仍按真实标签
  引用（idct16 为 `%3`）——结构化导入器已自动发现并改名；
- 已修复：addr 正则分组错位（group2=ptr 被当偏移）、store 类型解析、
  `<4 x i16>` 源交织（vcombine(vzip1,vzip2)）、GEP 元素乘数；
- **未解决**：门禁全失配（确定性，-O0/-O2 相同）。已排除：
  缺失 phi 边（0）、phi 引用未定义（0）、入口/首个 diamond 发射正确
  （逐条对照 IR 无误）。零输入测试产生非零输出 → 某下游路径取错值。
- 下一步调试：在生成 C 的 store 前打印行值，对照 IR 逐步收窄；
  或按行切分（先只验证第 0 行的 32 个输出）二分定位。
