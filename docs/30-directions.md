...
- **第一轮校准完成（2026-08-14，docs/34）**：6 个样本（dct8/idct16/
  idct32/interp8 path-B 三形状）。结论：920B 替换比率方向在替换形状
  接近真实时可信，幅度为有界估计（BtoS 高估 cand 工作量）；MCA 对
  SVE2p3 内核是更好的方向代理；idct16 分歧需 950/960 复测裁决。

**P2.5：sa8d16 减半门已达成（2026-08-14）**
- 尾部归约改 `svaddv_s16` 每组合计（9 条 → 4 条），fused 189 → **186
  （< 186.5）**，20k + TestBenchLite PASS；候选已固化。
- **920B 原生/替换均不可行**（2026-08-14 核实）：内核主体 48 条
  `cadd`（SVE2-only，`svcadd`），920B（SVE1）SIGILL；`cadd` 无 1:1
  替换（替换会破坏“指令数不变”的流无关条件），sa8d16 实机验收
  只能等 950/960。
