...
## 5. 实证否决（2026-08-13，实现前）

用 20k 随机 32x32（[-255,255]）做标量模拟：

| 舍入方案 | 与上游分歧率 |
| --- | ---: |
| pass1 舍入 + pass2 舍入（上游语义） | 0 |
| **pass1 不缩放/不取整，pass2 末尾一次 >>15** | **3.870%** |

结论：**pass1 舍入不可跨 pass 延迟**（舍入非线性 + 上游 pass2 消费的是
已舍入 s16 coef）；简单延迟舍入的分歧率（3.87%）远超 DCT16 legacy 可
验收阈值（0.045%），partial 直通在 upstream-exact 或 TestBench 门禁
合同下**不可行**。内部参考 4251 的 0.104% 分歧签名来自其专属
饱和窄化语义（docs/18），无法在不放宽合同的前提下复现。

因此 DCT32 在本项目合同下的现实收敛点 = **7190（v2，0.566x，
near-gate）**；半数门 6355 与内部 4251 需要 legacy-internal-exact
合同族（对齐内部 0.104% 签名）或 960 实机口径重新评估，暂列挂起。
剩余精力建议转向：interp8_hpp 接入评估 / dct8 迭代 / 960 实机校准。
