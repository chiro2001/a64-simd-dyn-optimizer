...
### 6.15 2026-08-14：内部 950 TestBench 实机 cycle（用户提供）

在内部 950（SVE2，VL=256；SVE 2×256 / NEON 4×128，与 920B 管道数
相同，**不是 960/NP1 的 4×256**）上跑 x265 TestBench 的 dct32 门禁，
实机 cycle（多次测量范围）：

| kernel | TestBench cycles | 相对 best_op_r16 |
| --- | ---: | ---: |
| **best_op_r16（工具生成）** | **1019 ~ 1077** | 1.00× |
| 内部手写算子 | 1167 | ~1.08× ~ 1.15× |
| 上游 dct32_sve | 2107 | ~1.96× ~ 2.07× |

结论：
- 工具生成的 best_op_r16 在 SVE2 实机上**超过内部手写算子 8%~15%**，
  较上游快约 **2×**；这是首个“工具搜索超过内部参考”的实机证据。
- 与 920G 之前“指令数减少但 cycle 无差异”的旧结论相反，本组数据
  说明在正确管线（SVE2/256）上指令数与 cycle 同步改善。
- 待办：把 4014（MCA 最优）与 3930/4002（fused 最优）在内部 950 上
  做同机 paired，确认 MCA 预测的 1041 vs 1094 是否兑现。

### 6.16 2026-08-14：全代理重排确认 4014（MCA/结构成本双第一）

在 m32 布局空间上做全代理重排（`--rank-by mca --mca-top 10
...
- **cp 使用边界（全量回填后修正）**：cp 只在同族 top-N 内做依赖链
  诊断；跨候选不可比——688 个候选里 cp 最低（57~60）的反而是
  fused_uop 5369~8206 的候选（并行度高、单链短），全局
...
### 6.17 2026-08-14：950 实机 paired 确认 best_op_mca 最快（用户实测）

用户提供 950 实机 TestBench cycles（**best_op_mca 985~995**）：

| kernel | 950 TestBench cycles | 相对 best_op_mca |
| --- | ---: | ---: |
| **best_op_mca（4014）** | **985 ~ 995** | 1.00× |
| best_op_r16（3930/4002） | 1019 ~ 1077 | ~1.03× ~ 1.09× |
| 内部手写算子 | 1167 | ~1.18× ~ 1.19× |
| 上游 dct32_sve | 2107 | ~2.12× ~ 2.14× |

结论：
- **cycle 代理选型被实机验证**：llvm-mca（neoverse-v2）预测 4014=
  1041，实测 985~995（约 +5% 高估），是当前最准的代理；结构成本
  （920B 1164 / NP1 728）偏差更大，只能做粗排。
- best_op_mca 较 best_op_r16 快约 3%~9%，较内部算子快 ~18%，
  较上游快 ~2.1×；工具搜索选出的 cycle-best 已确认为实机最快。
