...
| 3. 发射 | 当前 = NEON+SVE 混合（sv* + NEON bridge）；纯 SVE 模式 = 开关禁 NEON | ⚠️ 混合版已实现；纯 SVE 原语集 + **dct16 发射器已完成**（0 NEON、20k VL=128 差分 0 失配），dct32 待做 |
...
1. **纯 SVE 发射模式**：禁 NEON 门禁已落地（`check_isa_level
   --no-neon` + `AGO_PURE_SVE=1` 注入门禁）；纯 SVE 原语集与
   **dct16 发射器已完成**（0 NEON、20k VL=128 差分 0 失配、与 neon8
   数值一致）；**dct32 发射器待做**；当前限定 VL=128。
