...
## 5. 实际优化迭代结束后的单次顶级模型困难求助

这是一个**独立归档、一次性的顾问步骤**，不阻塞主体流水线，也不授权模型自行合入修改。它只在一次实际优化迭代已经完成实现/生成、正确性验证、benchmark 和本轮结论后触发；初始项目规划、环境审计、M0 基线建设以及纯文档整理均不触发专家审核。每个实际优化 iteration 最多发起一次请求、保存一次回复，不围绕该回复继续 `resume`、追问或发起多轮审阅。
...
顶级模型建议不是证明，也不自动改变 roadmap。接受的建议必须变成下一轮可证伪假设；拒绝或延期同样给出技术理由。每次实际优化迭代至多对应一个新目录，永不覆盖旧 `response.md`；`decision.md` 是本地处置记录，不是第二轮模型审核。
...
不能只说“tests pass”或“性能提升约 30%”；必须给出测试对象、baseline 和证据路径。
