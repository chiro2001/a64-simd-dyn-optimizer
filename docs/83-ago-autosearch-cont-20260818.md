...
2. **psy-cost 家族 920B 全形状实测**（SVE1 cover 全兼容）：
   ...
   - **机理修正**：8x8 cover 实为 **NEON 实现**（vaddl/vsubl/hadamard
     内联，非 SVE1——与 sa8d16 best_sve1 同情况），赢面是"生成版
     NEON vs x265 手写分派"的实机胜利：8x8 块小、x265 手写实现
     未特别优化小块，生成版指令调度更紧凑；大块计算密集时两者
     效率逼近（32/64 噪声内）。
