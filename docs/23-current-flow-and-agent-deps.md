...
**并发扫描（2026-08-14，本地 12 核 AMD EPYC 无 SMT，dct32 序列搜索
31 源/27 实测，全新目录）**：
...
| 1 | 118.1 s | 99 | 1.0× |
| 2 | 74.5 s | 154 | 1.58× |
| 4 | ~52-54 s | 218-220 | 2.2× |
| 8 | 47.7 s | 301 | 2.47× |
| 12 | 45.3 s | 381 | 2.60× |

结论：QEMU trace/差分在磁盘 IO 上约 4-5 个有效 worker 即饱和，
8 worker 达不到 5× 的乐观目标；这是机器饱和点而不是算法问题。
所有 W 的结果逐行一致（W=1/2/4/8/12 字段零差异），排序稳定。
