...
| 920B | 2（三臂） | +591.5 | **+2.66%** | [558.0, 647.0] | 1600c2fa… |

可加性（dct IR 相对 best9-noremain 的增量）：N1 第二批
+0.82% [222, 653]；710 第一批 +0.71% [61, 245]、第二批 +1.10%
[133, 331]；920B 同一轮三臂 **+0.59% [74.5, 169.0]**——CI 均不含
0（920B 三臂报告：reports/920b-best9-noremain-ir-dct-3arm-
20260817.txt）。
...
`reports/920b-best9-noremain-ir-dct-3arm-20260817.txt`。
...
## 5. M4 就绪度（2026-08-17 更新）

round-0026 M4 门：**四机均不回退，且至少两机相对 best9 额外
≥0.5pp、CI 不跨零**。

| 机器 | 冻结集 vs base | 相对 best9-noremain 增量 | 状态 |
| --- | --- | --- | --- |
| N1 | +2.14~2.25% | +0.82% [222, 653] | 双批 ✓ |
| 710 | +2.03~2.36% | +0.71~1.10% [61, 331] | 双批 ✓ |
| 920B | +2.63~2.66% | +0.59% [74.5, 169.0] | 双轮 ✓ |
| 950 | 30f dct 包 +0.79%（旧 op 包） | 待测 | 用户侧 |

三机已满足“不回退 + 额外 ≥0.5pp + CI 不跨零”；950 用冻结集跑
30f/100f（`AGO_IR_SVE16` 或既有 23-kernel 流程）后即可按 docs/52
撰写 M4 独立声明。

**核验器**：`tools/m4_declaration.py` 从 DB 读取冻结集 e2e 与增量
行（N1/920B/710 增量行已入库：-0.82% / -0.59% / -1.10%，CI 均不
跨零），输出 M4 判定表（`reports/m4-declaration-20260817.txt`）。
当前状态：不回退 3/4、额外 ≥0.5pp 3/2，950 待测——950 数据入库后
重跑即可得最终声明。
