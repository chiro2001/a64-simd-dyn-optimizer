...
## 47. vps/vsp/vss 32x16 补齐（2026-08-15）

- 验收（各 20k 差分 0 失配）：
  - vps 32x16：**424 fused / MCA 252**；
  - vsp 32x16：**1867 / 594**；
  - vss 32x16：**1736 / 546**。
- 冒烟测试扩到 61 核。

## 48. vps/vsp/vss 8x4 与 16x4 补齐（2026-08-15）

- 验收（各 20k 差分 0 失配）：
  - vps 8x4/16x4：**70/50、70/50**；
  - vsp 8x4/16x4：**125/75、241/110**；
  - vss 8x4/16x4：**116/68、224/100**。
- 冒烟测试扩到 67 核。

## 49. vsp/vss 行预加载优化（2026-08-15）

- `_emit_vertical_ss_native` / `_emit_vertical_sp_native` 改为一次性
  预加载 rows+7 行 s16 行向量（width16/32 每个 z.h 存 16 halfwords，
  32 宽两段），8-lane 组从预加载行用 `svunpklo/pi_s32` 加宽，消除逐
  tap 重复 load。
- 效果（20k 差分仍全部 0 失配）：
  - vss 16x16：872/297 → **450/291**，stack 288→45；
  - vsp 16x16：938/317 → **517/311**，stack 256→16；
