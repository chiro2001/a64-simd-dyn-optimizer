#!/usr/bin/env bash
# Freeze-and-replicate best6b on a target 920B/950 (round-0022 P0).
#
...
python3 tools/build_preload_so.py --isa sve1 \
  --kernels cost-c1c2-flag,cost-coeff-nxn,cost-coeff-remain,sa8d16,satd-8,scan-pos-last \
  --opt=-O3 \
  --inject-outdir build/dynopt-inject-best6b-freeze \
  --workdir build/preload-work-best6b-freeze \
  --json build/dynopt-inject-best6b-freeze/report.json

PKG=$(mktemp -d /tmp/e2e-pack.XXXXXX)
mkdir -p "$PKG/out" "$PKG/work"
cp build/dynopt-inject-best6b-freeze/*.patch \
   build/dynopt-inject-best6b-freeze/*.cpp \
   build/dynopt-inject-best6b-freeze/*.o \
   build/dynopt-inject-best6b-freeze/objects.txt "$PKG/out/"
cp build/preload-work-best6b-freeze/*.o "$PKG/work/"
