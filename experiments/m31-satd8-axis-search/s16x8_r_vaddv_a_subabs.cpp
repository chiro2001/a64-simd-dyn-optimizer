...
static int dynopt_satd_s16x8_r_vaddv_a_subabs_impl(const uint8_t* pix1, intptr_t sp1,
                         const uint8_t* pix2, intptr_t sp2)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        diff[i] = vreinterpretq_s16_u16(vsubl_u8(
            vld1_u8(pix1 + i * sp1 + 8),
            vld1_u8(pix2 + i * sp2 + 8)));
        diff[8 + i] = vreinterpretq_s16_u16(vsubl_u8(
            vld1_u8(pix1 + (i + 0) * sp1 + 0),
            vld1_u8(pix2 + (i + 0) * sp2 + 0)));
    }
    uint16x8_t out[4];
    ...
    return (int)(vaddvq_u16(s0) + vaddvq_u16(s1));
}

extern "C" int dynopt_satd_s16x8_r_vaddv_a_subabs(const uint8_t* pix1, intptr_t sp1,
                        const uint8_t* pix2, intptr_t sp2)
{
    return dynopt_satd_s16x8_r_vaddv_a_subabs_impl(pix1, sp1, pix2, sp2);
}
