...
## 5. 补测（M25 修复 harness 后复跑，2026-08-13）

M25 修复了微基准的输入分布、latency 共享序列、单 dst WAW 与 empty UB。
用修复后的 harness 重测同一 9 候选（`measurements-fixed-harness.json`）：

| 机器 | 全量拟合 Spearman | LOOCV Spearman | LOOCV R² |
| --- | ---: | ---: | ---: |
| 920B | 0.633 | -0.467 | -2.28 |
| N1 | **0.967** | -0.600 | -3.87 |

全量拟合在样本内更好（N1 0.97），留一法仍为负：回归过拟合、不能推广的
结论不变。
