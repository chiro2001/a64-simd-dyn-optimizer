// Generated by optimizer/ago/covers_satd_shapes.py -- do not edit.
#include <arm_neon.h>
#include <stdint.h>
\
...
extern "C" int dynopt_ago_satd16x8(const uint8_t* pix1, intptr_t stride1,
                  const uint8_t* pix2, intptr_t stride2)
{
    int16x8_t d[16];
    d[0] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 0 * stride1), vld1_u8(pix2 + 0 * stride2)));
    d[1] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 1 * stride1), vld1_u8(pix2 + 1 * stride2)));
    d[2] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 2 * stride1), vld1_u8(pix2 + 2 * stride2)));
    d[3] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 3 * stride1), vld1_u8(pix2 + 3 * stride2)));
    d[4] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 4 * stride1), vld1_u8(pix2 + 4 * stride2)));
    d[5] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 5 * stride1), vld1_u8(pix2 + 5 * stride2)));
    d[6] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 6 * stride1), vld1_u8(pix2 + 6 * stride2)));
    d[7] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 7 * stride1), vld1_u8(pix2 + 7 * stride2)));
    d[8] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 0 * stride1), vld1_u8(pix2 + 8 + 0 * stride2)));
    d[9] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 1 * stride1), vld1_u8(pix2 + 8 + 1 * stride2)));
    d[10] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 2 * stride1), vld1_u8(pix2 + 8 + 2 * stride2)));
    d[11] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 3 * stride1), vld1_u8(pix2 + 8 + 3 * stride2)));
    d[12] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 4 * stride1), vld1_u8(pix2 + 8 + 4 * stride2)));
    d[13] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 5 * stride1), vld1_u8(pix2 + 8 + 5 * stride2)));
    d[14] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 6 * stride1), vld1_u8(pix2 + 8 + 6 * stride2)));
    d[15] = vreinterpretq_s16_u16(vsubl_u8(vld1_u8(pix1 + 8 + 7 * stride1), vld1_u8(pix2 + 8 + 7 * stride2)));
    int16x8_t ta[4];
    hadamard_4_v(d + 0, ta);
    hadamard_abs_4_h(ta, d + 0);
    uint16x8_t maa = vmaxq_u16(vreinterpretq_u16_s16(d[0]),
                                vreinterpretq_u16_s16(d[1]));
    uint16x8_t mab = vmaxq_u16(vreinterpretq_u16_s16(d[2]),
                                vreinterpretq_u16_s16(d[3]));
    uint16x8_t oa = vaddq_u16(maa, mab);

    int16x8_t tb[4];
    hadamard_4_v(d + 4, tb);
    hadamard_abs_4_h(tb, d + 4);
    uint16x8_t mba = vmaxq_u16(vreinterpretq_u16_s16(d[4]),
                                vreinterpretq_u16_s16(d[5]));
    uint16x8_t mbb = vmaxq_u16(vreinterpretq_u16_s16(d[6]),
                                vreinterpretq_u16_s16(d[7]));
    uint16x8_t ob = vaddq_u16(mba, mbb);

    int16x8_t tc[4];
    hadamard_4_v(d + 8, tc);
    hadamard_abs_4_h(tc, d + 8);
    uint16x8_t mca = vmaxq_u16(vreinterpretq_u16_s16(d[8]),
                                vreinterpretq_u16_s16(d[9]));
    uint16x8_t mcb = vmaxq_u16(vreinterpretq_u16_s16(d[10]),
                                vreinterpretq_u16_s16(d[11]));
    uint16x8_t oc = vaddq_u16(mca, mcb);

    int16x8_t td[4];
    hadamard_4_v(d + 12, td);
    hadamard_abs_4_h(td, d + 12);
    uint16x8_t mda = vmaxq_u16(vreinterpretq_u16_s16(d[12]),
                                vreinterpretq_u16_s16(d[13]));
    uint16x8_t mdb = vmaxq_u16(vreinterpretq_u16_s16(d[14]),
                                vreinterpretq_u16_s16(d[15]));
    uint16x8_t od = vaddq_u16(mda, mdb);

    uint16x8_t accv = (oa + ob + oc + od);
